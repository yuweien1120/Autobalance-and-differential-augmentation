...
def main(config):
    ...
    # 定义训练损失函数参数的优化器
    if config.use_trained_params:
        with open(f'dy_IB{config.IB_rate[0]}.txt', "r") as f:
            dy = f.read()
            dy = torch.tensor(eval(dy)).to(device)
        with open(f'ly_IB{config.IB_rate[0]}.txt', "r") as f:
            ly = f.read()
            ly = torch.tensor(eval(ly)).to(device)
        with open(f'py_IB{config.IB_rate[0]}.txt', "r") as f:
            py = f.read()
            py = torch.tensor(eval(py)).to(device)
        with open(f'uy_IB{config.IB_rate[0]}.txt', "r") as f:
            uy = f.read()
            uy = torch.tensor(eval(uy)).to(device)
    else:
    ...
    up_optimizer = optim.SGD(params=[{'params': dy}, {'params': ly}], **config.up_optim_params)
    # 优化器的学习率策略
    ...
    for epoch in range(1, config.epochs + 1):
        acc = train(model, device, low_loader=low_loader,
                    ...
                    transforms_temperature=config.temperature, num_classes=config.num_classes, is_search=False)
...
if __name__ == "__main__":
    ...
    wandb.init(project="autobalance_loss_and_aug(retraining)", name=name)
    wandb.watch_called = False

    # 定义上传的超参数
    config = wandb.config

    config.use_trained_params = False  # 使用训练后的参数
    # 数据集及其预处理
    ...
    config.augmentations = ("AddGaussian", "Translation")
    ...
    config.up_lr = 0.1  # 顶层学习率(调整损失函数超参数的学习率)
