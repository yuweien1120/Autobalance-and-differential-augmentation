...
class AddGaussian(Augmentation):
    def __init__(self, p=0.5, mean=0, u=0.05, temperature=1.):
        """
        初始化高斯噪声相关参数
        :param p: 使用数据增强的概率(float or shape[batch_size])
...
class RandomScale(Augmentation):
    # 随机信号缩放
    def __init__(self, p=0.5, u=0.05, temperature=1.):
        super().__init__(p, temperature)
        self.u = u
...
if __name__ == '__main__':
    transforms_name = ('AddGaussian', 'MaskNoise', 'RandomScale', 'Translation')
    p = torch.ones([4, 10], requires_grad=True, device='cuda')
    u = torch.ones([4, 10], requires_grad=True, device='cuda')
    print(p)
    temperature = 1.
    index = torch.ones([60], dtype=torch.long)
    p = params_index(index, p)
    u = params_index(index, u)
    transforms_params_dict = generate_params_dict(transforms_name, p, u)
    print(transforms_params_dict)
